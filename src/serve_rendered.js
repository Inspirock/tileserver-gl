...
const extractMarkersFromQuery = (query, transformer) => {
  const markerParts = (query.markers || '').split('|');
  const markers = [];
  for (const pair of markerParts) {
    const pairParts = pair.split(',');
    if (pairParts.length == 2) {
      let pair;
      if (query.latlng == '1' || query.latlng == 'true') {
...
const renderOverlay = (z, x, y, bearing, pitch, w, h, scale,
  ...
  for (const pair of markers) {
    const coordinates = precisePx(pair, z);
    // draw circle
    ctx.strokeStyle = '#000';
    ctx.lineWidth = 1.5;
    ctx.beginPath();
...
const markerWidth = 36,
  markerHeight = 36,
  markerImage = createCanvas(markerWidth, markerHeight);
...
module.exports = {
  init: (options, repo) => {
    ...
    const markerLoadPromise = new Promise((resolve, reject) => {
      loadImage(__dirname + '/map-marker.png').then((img) => {
        const ctx = markerImage.getContext("2d");
        ctx.drawImage(img, 0, 0, img.width, img.height, 0, 0, markerImage.width, markerImage.height);
        resolve();
      }, (reason) => {
        reject();
      });
    ...
    return Promise.all([fontListingPromise, markerLoadPromise]).then(() => app);
  },
